...
- 🎯 Entry points to execute your program using `python -m <lam>` or `$ lam` with basic CLI argument parsing.
...
---
# lam Flask Application

Awesome lam created by ccxx72
...
```bash
git clone https://github.com/ccxx72/lam lam
cd lam
make install
```
...
```bash
lam create-db   # run once
lam populate-db  # run once (optional)
lam add-user -u admin -p 1234  # ads a user
lam run
```
